...
class post extends MY_Controller {
	...
	{
		$img_folder = './themes/default/uploads/post/';

		if ($id == NULL)
		{
			$this->data['subtitle'] = 'Create post';
			$post                   = $this->post_m->_new();
			$post->images           = 'placeholder.jpg';
		...
		if ($this->form_validation->run($this) == TRUE)
		{
		    ...
		    {
		    	$_POST[$fieldname] = $post->images;
		    }

		    if (!empty($_POST[$fieldname])) {

		    	$data  = $this->post_m->array_form_post(array('catid','title','slug','intro','body','tags','images','created','createdby','modified','modifiedby','status','pubdate','comment'));
		...
		$this->data['post_title'] = array(
			...
			'required' => 'required',
			'class' => 'form-control',
			'placeholder'=> $this->lang->line('post_title_placeholder'),
		...
		$this->data['slug'] = array(
			...
			'placeholder'=> $this->lang->line('post_slug'),
			'value' => $this->form_validation->set_value('slug', $post->slug),
		);
		...
		$this->data['intro'] = array(
			...
			'class'    => 'form-control',
			'rows' => '5',
			'value' => $this->form_validation->set_value('intro', $post->intro),
		);
		...
		$this->data['body'] = array(
			...
			'id'    => 'editor-full',
			'required' => 'required',
			'value' => $this->form_validation->set_value('body', $post->body),
		);
		...
		$this->data['publication_date'] = array(
			...
			'placeholder'=> $this->lang->line('post_publish_date'),
			'value' =>  $post->pubdate,
		);
		...
		$this->data['status_selected'] = $post->status;
		...
		foreach ($this->post_m->_get_category() as $key => $value) {
		 	$this->data['cat_option'][$value->id] = $value->name;
		 };
		$this->data['cat_selected'] = $post->catid;
		...
		$this->data['tags'] = array(
			...
			'value' => $this->form_validation->set_value('tags', $post->tags),
		);
		...
		$this->data['featured_images'] = array(
			...
			'type'  => 'file',
			'class' => 'file-input',
			'placeholder'=> ($post->images == 'placeholder.jpg' ? $this->lang->line('general_no_file') : $post->images) ,
			'data-src' => $this->config->item('base_url') . $img_folder . $post->images,
			'value' => $this->form_validation->set_value('images', $post->images),
		);
		...
		$this->data['comment_selected'] = $post->comment;

		$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
		$this->parser->parse('post_edit', $this->data);
	}

	public function delete($id)
	{
		$img = $this->post_m->get($id);

		if ($img->images)
		{
			$filename = './themes/default/uploads/post/' . $img->images;
			if (file_exists($filename) && $img->images != 'placeholder.jpg') {
			    unlink($filename);
			}
		}

		if ($this->post_m->delete($id))
		{
	...
	public function create_category()
	{
		...
		$data  = $this->post_m->array_form_post(array('name','description','slug','created','createdby'));

		if ($this->post_m->_save_category($data))
		{
			$msg = array("msg" => $this->lang->line('post_add_category_success'));
			echo json_encode($msg);
		}
		else
		{
			$msg = $this->lang->line('post_add_category_failed');
			echo json_encode($msg);
		}
	}
	...
	public function _unique_slug($str)
	{
		...
		$page = $this->post_m->check_exists('slug','slug',$id);
