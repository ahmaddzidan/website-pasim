...
class Img
{
	 ...
	 * @return string
	 */
	public function rimg($controller, $source, $params, $no_cache = FALSE)
	{
		$source = trim($source);

		if (file_exists($this->ci->config->config['img']['base_path'] . $controller . '/' . $source))
		{
			$source = $this->ci->config->config['img']['base_path'] . $controller . '/' . $source;
		}
		else
		{
		...
		if (! is_file($source))
		{
			return '<img src="http://placehold.it/' . $params['width'] . 'x' . $params['height'] . '" width="'.$params['width'].'" height="'.$params['height'].'" alt="noimage '.$source.'" class="'.$params['class'].'">';
			die();
		}
		...
		if (! is_dir($this->ci->config->config['img']['base_path'] . $controller . $dir))
		{
			if (is_writable($this->ci->config->config['img']['base_path'] . $controller))
			{
				mkdir($this->ci->config->config['img']['base_path'] . $controller . $dir);
			}
			else
			{
				die('Unable to create cache dir in ' . $this->ci->config->config['img']['base_path'] . $controller);
			}

		}

		// full path to final file
		$dst['file'] = $this->ci->config->config['img']['base_path'] . $controller . $dir . "/" . basename($source);
		...
		if ($src['width'] === $dst['width'] && $src['height'] === $dst['height'])
		{
			...
			if ($params['return']) {
				...
				return '<img src="' . base_url($this->ci->config->config['img']['base_url'] . $controller . "/" . basename($dst['file'])) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
		...
		if (file_exists($dst['file']) AND $params['nocache'] !== TRUE)
		{
			...
			if ($params['return']) {
				return "{$this->ci->config->config['img']['base_url']}{$controller}{$dir}/" . basename($dst['file']);
			} else {
				return "<img src=" . base_url($this->ci->config->config['img']['base_url'] . $controller . $dir . '/' . basename($dst['file'])) . " width=\"{$params['width']}\" height=\"{$params['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
